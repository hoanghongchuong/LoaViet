...
class IndexController extends Controller
{
    ...
    */
    ...
    public function updatedblaravel(){
        $cateHots = DB::table('s_danhmuc')->get();

        foreach($cateHots as $k=>$item){
            
            $news = new ProductCate;
            $news->parent_id = 0;
            $news->id = $item->s_madanhmuc;
            $news->name = $item->s_tendanhmuc;
            $news->alias = $item->s_link;
            $news->title = $item->s_title;
            $news->photo = $item->s_hinhanh;
            $news->keyword = $item->s_keyword;
            $news->description = $item->s_description;
            $news->content = $item->s_content;
            $news->stt = $k;
            $news->status = 1;
            $news->com = 'san-pham';
            $news->save();
    ...
    public function updateCateChild(){
        $cateChilds = DB::table('s_loaisanpham')->get();
        foreach($cateChilds as $k=>$item){

            
            $news = new ProductCate;
            $news->parent_id = $item->s_madanhmuc;
            
            $news->name = $item->s_tenloaisp;
            $news->alias = $item->s_link;
            $news->title = $item->s_title;
            $news->photo = $item->s_hinhanh;
            $news->keyword = $item->s_keyword;
    ...
    public function updateproudct(){
        $products = DB::table('s_sanpham')->get();
        foreach($products as $item){
            $data = new Products;
            $data->name = $item->s_tensp;
            $data->cate_id = $item->s_maloaisp;
            $data->alias = $item->s_link;
            $data->photo = $item->s_hinhanhsp;
            $data->keyword = $item->s_keyword;
            $data->description = $item->s_description;
            $data->title = $item->s_title;
            $data->nhacc = $item->s_nhaccsp;
            $data->price = $item->s_giasp;
            $data->trongluong = $item->s_trongluong;
            $data->baohanh = $item->s_baohanh;
            $data->viewer = $item->s_viewer;
            $data->noibat = $item->s_hot;
            $data->spbc = $item->s_sale;
            $data->content = $item->s_content;
            $data->mota = $item->s_info;
            $data->com = 'san-pham';
            $data->save();
        }
    }
    public function updatecatenews(){
        $cateNews = DB::table('s_subnews')->get();
        foreach($cateNews as $k=>$item){
            
            $news = new NewsCate;
            
            $news->id = $item->s_maloaitin;
            $news->name = $item->s_tenloaitin;
            $news->alias = $item->s_link;
            $news->title = $item->s_title;
            $news->keyword = $item->s_keyword;
            $news->description = $item->s_description;
    ...
    public function updatenews(){
        $cateNews = DB::table('s_news')->get();
        foreach($cateNews as $k=>$item){
            
            $news = new News;
            
            
            $news->name = $item->s_tentin;
            $news->cate_id = $item->s_maloaitin;
            $news->photo = $item->s_image;
            $news->noibat = $item->s_hot;
            $news->alias = $item->s_link;
            $news->title = $item->s_title;
            $news->keyword = $item->s_keyword;
            $news->description = $item->s_description;
    ...
    public function index()
    {
        ...
        $tintuc_moinhat = DB::table('news')->select()->where('status', 1)->where('com', 'tin-tuc')->orderBy('id', 'desc')->take(2)->get();
        ...
        $cateNews = DB::table('news_categories')->where('status',1)->where('com','tin-tuc')->orderBy('id', 'desc')->get();
        $news_products = DB::table('products')->select()->where('status', 1)->where('com','san-pham')->orderBy('id', 'desc')->take(16)->get();

        $hot_product = DB::table('products')->where('status', 1)->where('noibat', 1)->where('com','san-pham')->orderBy('id', 'desc')->take(16)->get();
        ...
        return view('templates.index_tpl', compact('banner_danhmuc', 'com', 'khonggian_list', 'about', 'tintuc_moinhat', 'keyword', 'description', 'title', 'img_share', 'hot_news', 'news_products', 'productSalling', 'slider', 'banner_sidebar', 'cateHots', 'productNoiBat', 'cate_pro', 'productSale','feedback','cates','hot_product','cateNews'));
    }
        ...
        if (!empty($product_cate)) {
            ...
            if ($req->sort) {
                if (isset($this->sortType[$req->sort])) {
                    ...
                    $appends['sort'] = $req->sort;
                    $products = $products->where('com','san-pham')->orderBy($this->sortType[$req->sort]['order'][0], $this->sortType[$req->sort]['order'][1]);
                }
            }
            $products = $products->orderBy('id', 'desc')->paginate(24);
            if (count($appends)) {
    ...
    public function getProductDetail($id)
    {
        ...
        if (!empty($product_detail)) {
            $banner_danhmuc = DB::table('lienket')->select()->where('status', 1)->where('com', 'chuyen-muc')->where('link', 'san-pham')->get()->first();
            $product_khac = DB::table('products')->select()->where('status',1)->where('com','san-pham')->where('alias','<>',$id)->orderby('id','desc')->take(3)->get();
            ...
            $productSameCate = DB::table('products')->select()->where('status', 1)->where('alias', '<>', $id)->where('cate_id', $product_detail->cate_id)->where('com','san-pham')->orderby('stt', 'desc')->take(12)->get();
            $setting = Cache::get('setting');
            $tintucs = DB::table('news')->orderBy('id', 'desc')->take(3)->get();
            // Cấu hình SEO
    ...
    public function search(Request $request)
    {
        ...
        // End cấu hình SEO
        $productNew = DB::table('products')->select()->where('status',1)->orderBy('stt','desc')->take(12)->get();
        $products = DB::table('products')->select()->where('name', 'LIKE', '%' . $search . '%')->where('com','san-pham')->orderBy('id', 'DESC')->paginate(20);
        // dd($products);
    ...
    public function getNews()
    {
        ...
        $cate_pro = DB::table('product_categories')->where('status', 1)->where('parent_id', 0)->orderby('id', 'asc')->get();
        // dd($cateNews);
        $tintucs = DB::table('news')->select()->where('status', 1)->where('com', 'tin-tuc')->orderby('id', 'desc')->paginate(12);
    ...
    public function getListNews($alias){
        ...
        $newsByCate = DB::table('news')->where('com','tin-tuc')->where('cate_id', $newsCate->id)->paginate(6);
        if($newsCate->title){
        ...
        return view('templates.listnews', compact('cateNews','tintuc_noibat','newsByCate','title','keyword','description','newsCate'));
    }
    ...
    public function getNewsDetail($id)
    {
        $cateNews = DB::table('news_categories')->where('com', 'tin-tuc')->get();
        $cate_pro = DB::table('product_categories')->where('status', 1)->where('parent_id', 0)->orderby('id', 'desc')->get();
        $tintuc_noibat = DB::table('news')->select()->where('status', 1)->where('noibat', '>', 0)->where('com', 'tin-tuc')->take(3)->orderBy('id', 'desc')->get();
        ...
        if (!empty($news_detail)) {
            ...
            $samNews = DB::table('news')->where('status',1)->where('cate_id', $news_detail->cate_id)->where('id', '<>', $news_detail->id)->take(4)->orderBy('id','desc')->get();
            $hot_news = DB::table('news')->where('status', 1)->where('noibat', 1)->orderBy('created_at', 'desc')->take(5)->get();
            $cate_pro = DB::table('product_categories')->where('status', 1)->where('parent_id', 0)->orderby('id', 'asc')->get();
            ...
            return view('templates.news_detail_tpl', compact('news_detail', 'com', 'tintuc_moinhat_detail', 'camnhan_khachhang', 'banner_danhmuc', 'baiviet_khac', 'quangcao_tintuc', 'keyword', 'description', 'title', 'img_share', 'hot_news', 'tinkhac', 'cate_pro','cateNews','tintuc_noibat','hotProducts','cate_pro','cateNews','samNews'));
        } else {
            return redirect()->route('getErrorNotFount');
        }
    ...
    public function addCartAjax(Request $req)
    {
         ...
         try {
            ...
            $carts = Cart::Content();
            return view('templates.ajaxCart', compact('carts'));
        } catch (\Exception $e) {
            return 0;
        }
    ...
    public function getDaiLy(){
        $about = DB::table('about')->where('com','huong-dan')->first();
        return view('templates.daily', compact('about'));
    }
    public function getVideo(){
        $videos = DB::table('video')->orderBy('id','desc')->paginate(8);
        $videoHot = DB::table('video')->where('noibat',1)->first();
        return view('templates.video', compact('videos', 'videoHot'));
    }
    public function getDetailVideo($alias){
        $data = DB::table('video')->where('alias', $alias)->first();
        return view('templates.detailVideo', compact('data'));
    }
    public function traGop(){
        $data = DB::table('about')->where('com', 'tra-gop')->first();
        // dd($data);
        return view('templates.tragop', compact('data'));
    }
}
