...
class ProductController extends Controller
{
    ...
    public function postAdd(ProductRequest $request)
    {
        ...
        if($request->hasFile('fImages')){
            ...
            $watermark = Image::make('upload/logo.png');                
        ...
        if ($request->hasFile('detailImg')) {
            ...
            foreach ($request->file('detailImg') as $file) {
                ...
                if (isset($file)) {
                    ...
                    $slug= str_slug($request->name).'-'.str_random(4);
                    $key=0;
                    $fileName = "img-".$slug."-".$key. "." . strtolower($file->getClientOriginalExtension());
                    $destinationPath = 'upload/hasp';                  
                    $img = Image::make($file->getRealPath());
                    $watermark = Image::make('upload/logo.png');
                    $img->insert($watermark, 'bottom-right', 10, 10);
                    $img->save($destinationPath.'/'.$fileName);

                    $product_img->photo = $fileName;
